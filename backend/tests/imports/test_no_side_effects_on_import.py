...
CRITICAL_MODULES = [
    "backend.app.core.agents.caculinha_bi_agent",
    "backend.app.core.agents.orchestrator",
    "backend.app.core.llm_factory", # Factory crucial
    "backend.app.infrastructure.database.relational_db",
]
...
def clean_sys_modules():
    ...
    to_remove = [m for m in sys.modules if m.startswith("backend.app.")]
    for m in to_remove:
        del sys.modules[m]
...
def test_no_sys_exit_on_import(clean_sys_modules):
    ...
    with patch.object(sys, 'exit', side_effect=SystemExit("SYS_EXIT_CALLED")) as mock_exit:
        try:
            ...
            import backend.app.core.prompts.master_prompt
...
def test_no_global_code_execution():
    ...
    if "backend.app.core.llm_factory" in sys.modules:
        del sys.modules["backend.app.core.llm_factory"]
    if "backend.app.core.llm_groq_adapter" in sys.modules:
        del sys.modules["backend.app.core.llm_groq_adapter"]
        
    # Mockamos a classe GroqLLMAdapter onde ela é definida
    with patch("backend.app.core.llm_groq_adapter.GroqLLMAdapter") as mock_groq_adapter:
         
        # Importa o módulo diretamente
        import backend.app.core.llm_factory as llm_factory
        ...
        assert hasattr(llm_factory, "SmartLLM")
