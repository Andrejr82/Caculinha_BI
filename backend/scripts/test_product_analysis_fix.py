...
def main() -> int:
    ...
    try:
        ...
        print("[OK] PASS: Produto encontrado!")
    ...
    try:
        ...
        result = gerar_grafico_universal_v2.invoke(
            {"descricao": "vendas mensais", "filtro_produto": "369946", "tipo_grafico": "line"}
        )
        ...
        if result.get("status") == "error":
            print("[OK] WARNING: Grafico retornou erro (pode ser esperado se dados mensais nao existem)")
            print(f"   Mensagem: {result.get('message')}")
        elif "plotly_json" in result or "fig" in result or "chart_json" in result:
            print("[OK] PASS: Grafico gerado com sucesso!")
            print(f"   Titulo: {result.get('titulo', result.get('title', 'N/A'))}")
        else:
            print("[OK] PASS: Funcao executou sem erro (validacao estrutural)")
    ...
    try:
        ...
        result = buscar_produtos_inteligente.invoke({"descricao": "TNT 40 gramas preto", "limite": 3})

        assert result["status"] == "success", f"Erro na busca: {result.get('message')}"
        assert len(result["produtos"]) > 0, "Nenhum produto encontrado"

        print("[OK] PASS: Busca semantica funcionando!")
        ...
        print("[OK] SKIP: Dependencia RAG nao instalada (langchain-google-genai)")
        print("   Para instalar: pip install langchain-google-genai")
    except Exception as e:
        print(f"[FAIL] FAIL: {e}")
        import traceback
